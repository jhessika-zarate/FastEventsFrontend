<template>
  <header>
    <div class="overlay">
      ...
      <h3>Adminsitra los usuarios con el rol de técnico</h3>
      <br />
    </div>
  </header>
  <div>
    <form @submit.prevent="handleSearch">
      <div class="fx fx-gap">
        <div>
          <input type="text" placeholder="Buscar técnico" v-model="search" />
        </div>
        <div id="search-icon">
          <button type="submit">
        ...
        <div class="boton">
          <button id="add-button" @click="crearUsuario">Añadir</button>
        </div>
      </div>
    </form>
  </div>
  ...
  <div class="main-container_all">
    <div
      class="card"
      v-for="(conferencista, index) in filteredConferencistas"
      :key="index"
    >
      ...
      <div class="card-text">
        ...
        <div class="title-total">
          <div class="title">Técnico</div>
          <h2>{{ conferencista.nombre }} {{ conferencista.apellido }}</h2>
          <div class="desc">
            <i class="fas fa-envelope"></i
            ><span class="desc">{{ conferencista.correo }}</span>
          </div>

          <div class="desc">
            <i class="fas fa-phone"></i
            ><span class="desc">+591 {{ conferencista.telefono }}</span>
          </div>
          <div class="actions">
            <!--icono de editar -->
  ...
  <div class="modal_base">
    ...
    <div class="subtitle">{{ isEditing ? 'Modifica los campos necesarios' : 'Llena los campos para añadir un técnico' }}</div>
...
export default {
    ...
    async fetchConferencistas() {
      try {
        ...
        this.Conferencistas = response.filter(
          (usuario) => usuario.rol === 2 || usuario.rol === "2"
        );
    ...
    async aprobar(conferencista) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          await this.userStore.updateUsuarioRole(
            conferencista.idusuario,
            conferencista.rol
          );
          this.fetchConferencistas();
        }
  ...
  async handleSubmit() {
    try {
      const usuarioData = {
        ...this.formData,
        rol: this.isEditing ? parseInt(this.formData.rol) : 2
      };
