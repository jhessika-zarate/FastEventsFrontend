<template>
  <div class="banner" style="background-color: blue">
    ...
    <div
      v-if="themes[conference.estilo].type === 'wave'"
      class="wave-container"
    >
      <svg class="wave" viewBox="0 0 1440 320">
        <path
          :d="themes[conference.estilo].path"
          :fill="themes[conference.estilo].color"
        />
      </svg>
    </div>
...
export default {
  ...
  methods: {
    ...
    openConferencia() {
      ///conferencista/detalle' sin parametro
      this.$router.push({
        name: "detalleEvento",
      });
    },
  ...
  setup(props) {
    ...
    const openConferencia = async () => {
      ...
      const fechaConferencia = new Date(
        `${props.conference.fecha}T${props.conference.hora}`
      );
      ...
      if (ahora >= fechaConferencia) {
        if (props.conference.activo === 4) {
          try {
            await conferenciaStore.updateConferenciaEstado(
              props.conference.idconferencia,
              1
            );
